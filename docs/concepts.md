...
You can also build an "uber" image which contains the `bee` CLI in addition to your BPF programs. The end result is a standard docker image which can be distributed via standard docker-like workflows to run your BPF program anywhere that can run containers, such as a K8s cluster. Note that you will need sufficient capabilities to run the image, as loading and running the BPF program is a privileged operation for most intents and purposes.

An example workflow is as follows:
```bash
$ bee build --uber examples/tcpconnect/tcpconnect.c tcpconnect
