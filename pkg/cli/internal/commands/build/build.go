...
func addToFlags(flags *pflag.FlagSet, opts *buildOptions) {
	...
	flags.BoolVar(&opts.Package, "package", true, "Package output binary into OCI image")
...
If you would prefer to build only the object file, you can include the '--package=false',
in which case you do not need to specify a registry:
$ build INPUT_FILE --package=false
...
func build(ctx context.Context, args []string, opts *buildOptions) error {
	...
	if len(args) == 1 {
		return fmt.Errorf("must specify a registry to package the output or run with '--package=false'")
	}
